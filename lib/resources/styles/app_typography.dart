import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:google_fonts/google_fonts.dart';

class AppTypography {
  // Light
  static TextStyle kLight10 =
      GoogleFonts.poppins(fontSize: 10.sp, fontWeight: FontWeight.w300);

  static TextStyle kLight12 =
      GoogleFonts.poppins(fontSize: 12.sp, fontWeight: FontWeight.w400);

  static TextStyle kLight14 =
      GoogleFonts.poppins(fontSize: 14.sp, fontWeight: FontWeight.w400);

  static TextStyle kLight20 =
      GoogleFonts.poppins(fontSize: 20.sp, fontWeight: FontWeight.w400);

  static TextStyle kLight22 =
      GoogleFonts.poppins(fontSize: 22.sp, fontWeight: FontWeight.w400);

  static TextStyle kLight24 =
      GoogleFonts.poppins(fontSize: 24.sp, fontWeight: FontWeight.w400);

  // Medium
  static TextStyle kMedium10 =
      GoogleFonts.poppins(fontSize: 10.sp, fontWeight: FontWeight.w500);

  static TextStyle kMedium12 = GoogleFonts.poppins(
      fontSize: 12.sp, fontWeight: FontWeight.w500, letterSpacing: 0.0);

  static TextStyle kMedium14 =
      GoogleFonts.poppins(fontSize: 14.sp, fontWeight: FontWeight.w500);

  static TextStyle kMedium16 =
      GoogleFonts.poppins(fontSize: 16.sp, fontWeight: FontWeight.w500);

  static TextStyle kMedium18 =
      GoogleFonts.poppins(fontSize: 18.sp, fontWeight: FontWeight.w500);

  static TextStyle kMedium20 =
      GoogleFonts.poppins(fontSize: 20.sp, fontWeight: FontWeight.w500);

  static TextStyle kMedium22 =
      GoogleFonts.poppins(fontSize: 22.sp, fontWeight: FontWeight.w500);

  //Bold

  static TextStyle kBold8 =
      GoogleFonts.poppins(fontSize: 8.sp, fontWeight: FontWeight.w600);

  static TextStyle kBold10 = GoogleFonts.poppins(
      fontSize: 10.sp, fontWeight: FontWeight.w600, letterSpacing: 0.0);

  static TextStyle kBold11 = GoogleFonts.poppins(
      fontSize: 11.sp, fontWeight: FontWeight.w600, letterSpacing: 0.0);

  static TextStyle kBold12 =
      GoogleFonts.poppins(fontSize: 12.sp, fontWeight: FontWeight.w600);

  static TextStyle kBold14 =
      GoogleFonts.poppins(fontSize: 14.sp, fontWeight: FontWeight.w600);

  static TextStyle kBold16 =
      GoogleFonts.poppins(fontSize: 16.sp, fontWeight: FontWeight.w600);

  static TextStyle kBold18 =
      GoogleFonts.poppins(fontSize: 18.sp, fontWeight: FontWeight.w600);

  static TextStyle kBold20 =
      GoogleFonts.poppins(fontSize: 20.sp, fontWeight: FontWeight.w600);

  static TextStyle kBold30 =
      GoogleFonts.poppins(fontSize: 30.sp, fontWeight: FontWeight.w600);

  // Extra Bold
  static TextStyle kExtraBold12 = GoogleFonts.poppins(
      fontSize: 12.sp, fontWeight: FontWeight.w700, letterSpacing: 0.0);

  static TextStyle kExtraBold14 = GoogleFonts.poppins(
      fontSize: 14.sp, fontWeight: FontWeight.w700, letterSpacing: 0.0);

  static TextStyle kExtraBold16 =
      GoogleFonts.poppins(fontSize: 16.sp, fontWeight: FontWeight.w700);

  static TextStyle kExtraBold18 =
      GoogleFonts.poppins(fontSize: 18.sp, fontWeight: FontWeight.w700);

  static TextStyle kExtraBold20 =
      GoogleFonts.poppins(fontSize: 20.sp, fontWeight: FontWeight.w700);

  static TextStyle kExtraBold24 =
      GoogleFonts.poppins(fontSize: 24.sp, fontWeight: FontWeight.w700);

  static TextStyle kExtraBold26 =
      GoogleFonts.poppins(fontSize: 26.sp, fontWeight: FontWeight.w700);

  static TextStyle kExtraBold22 =
      GoogleFonts.poppins(fontSize: 22.sp, fontWeight: FontWeight.w700);

  static TextStyle kCoverFont =
      GoogleFonts.patrickHand(fontSize: 28.sp, fontWeight: FontWeight.w700);
}
